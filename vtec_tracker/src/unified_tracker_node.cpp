...
void start_tracking()
{
  ...
  unified_optimizer->setHomography(H);
  alpha = 1.0;
  beta = 0.0;

  unified_optimizer->setReferenceTemplate(cur_img, BBOX_POS_X, BBOX_POS_Y, BBOX_SIZE_X, BBOX_SIZE_Y);
  cv::Mat reference_template;
  unified_optimizer->getReferenceTemplate(reference_template);
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    cv::imwrite("/home/mestrado/datasets/" + std::to_string(image_index++) + ".jpg", cur_img);
    ...
    if (state != STARTING)
    {
      zncc = unified_optimizer->optimize(cur_img, H_test, alpha_test, beta_test, VTEC::ZNCC_PREDICTOR);
      ...
      unified_optimizer->getCurrentTemplate(current_template);
    ...
    // }
    // if (!((state == NOT_TRACKING && zncc > 0.5) || (state == TRACKING && zncc > 0.0)) && (state != STARTING))
    // {
    //   state = NOT_TRACKING;
    // }
